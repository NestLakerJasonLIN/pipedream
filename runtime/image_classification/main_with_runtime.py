...
parser.add_argument(
    '--loss_scale', type=float, default=1,
        ...
        self.tensor = Variable(
            torch.rand(
                *
                input_size)).type(
            torch.FloatTensor)
        self.target = torch.Tensor(1).random_(
            0, num_classes)[0].type(
            torch.LongTensor)
        self.length = length
...
def main():
    ...
    if args.config_path is not None:
        ...
        configuration_maps['stage_to_rank_map'] = json_config_file.get(
            "stage_to_rank_map", None)
        configuration_maps['stage_to_rank_map'] = {int(k): v for (
            k, v) in configuration_maps['stage_to_rank_map'].items()}
    ...
    # num_ranks needed to determine number of warmup_minibatches in case of
    # pipelining
    args.stage = r.stage
...
def train(train_loader, r, optimizer, epoch):
    ...
    for i in range(n - num_warmup_minibatches):
        ...
        if is_last_stage():
            ...
            if i % args.print_freq == 0:
                print(
                    'Epoch: [{0}][{1}/{2}]\tMemory: {memory:.3f} ({cached_memory:.3f})'.format(
                        epoch,
                        i,
                        n,
                        memory=(
    ...
    print(
        "Epoch start time: %.3f, epoch end time: %.3f" %
        (epoch_start_time, time.time()))
...
def adjust_learning_rate(
    ...
    if args.lr_warmup and epoch < 5:
        lr = stage_base_lr * float(
            1 + step + epoch * epoch_length) / (5. * epoch_length)
